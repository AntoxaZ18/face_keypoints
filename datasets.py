...
class FaceKeyPointDataset(Dataset):
    def __init__(self, image_path: str, json_path: str, transform = None):
        ...
        self.annotations = {int(k): v for k, v in json.load(open(json_path, 'r', encoding="utf-8")).items()} 
    ...
    def __getitem__(self, index):
        ...
        landmarks = np.array([x / 2 for x in landmarks[::2]] + [y / 2 for y in landmarks[1::2]])
        image, landmarks = self.apply_transform(image, landmarks)

        landmarks = np.array([x / 256 for x in landmarks[::2]] + [y / 256 for y in landmarks[1::2]])
...
def create_dataset(images_src, images_dst, annotations, indexes):
    new_annotations = {k: v for k, v in annotations.items() if k in indexes} #filter test annotation
    ...
    bar = tqdm(indexes)
    for key in bar:
        shutil.copy(f"{images_src}/images/{annotations[key]['file_name']}", f"{images_dst}/images/{annotations[key]['file_name']}")
    
    new_annotations = {k: v for k, v in zip(range(len(new_annotations)), new_annotations.values())}  #recreate index
...
def train_test_split(original_path, train_path, test_path, original_json:str, test_size):
    import random 
    random.seed(42)
    annotations = {int(k): v for k, v in json.load(open(original_json, 'r', encoding="utf-8")).items()} 

    test_keys = set(random.sample(list(annotations.keys()), int(test_size * len(annotations))))
